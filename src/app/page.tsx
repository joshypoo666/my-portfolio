...
type SubProject = { title: string; challenge?: string; challengeBlocks?: ChallengeBlock[]; challengeImageUrl?: string; processImages?: string[]; processLabel?: string; processHeading?: string; processDescription?: string; process?: string; solution?: string; outcomes?: { stats: Outcome[]; reflection?: string; reflectionHeading?: string; reflectionItems?: string[] } };
type Outcome = { stat: string; label: string };
type Project = { title: string; category: string; year: string; tools?: string; client?: string; tagline?: string; challenge?: string; processHeading?: string; processDescription?: string; processPostText?: { heading: string; body: string }[]; heroImageUrl?: string; challengeImageUrl?: string; processImages?: string[]; subProjects?: SubProject[]; outcomes?: { stats: Outcome[]; reflection?: string; reflectionHeading?: string; reflectionItems?: string[] }; embedUrl?: string; thumbnailUrl?: string; thumbnailPosition?: string };
    ...
    <div className="max-w-5xl mx-auto px-6 py-10 pb-24">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-[#1e2e1e] rounded-xl overflow-hidden mb-16">
        {[
          ...
          { label: "Tools", value: project.tools ?? "[ Tools ]" },
      ...
      {/* Process */}
      <section className="mb-20">
        <p className="text-xs tracking-[0.25em] uppercase text-[#4a6a4a] mb-3">{activeSub?.processLabel ?? "02 — Debate"}</p>
        <h2 className="text-2xl font-semibold mb-5">{activeSub ? (activeSub.processHeading ?? `How We Approached ${activeSub.title}`) : "How We Got There"}</h2>
        ...
        {activeSub?.title === "Photo Reports" && (
          <section className="mb-20 mt-12">
            ...
            <div className="space-y-6">
            <h2 className="text-2xl font-semibold mb-5">User Understanding</h2>
            <p className="text-[#a8d8a8] leading-relaxed text-sm max-w-2xl">
              We knew from usage data that contractors weren't touching Photos or Forms the way they used CompanyCam — but we didn't know why. So we ran a series of moderated research calls, pulling in customers across different company sizes and roles: field leads, office managers, salespeople, owners. The goal was simple: stop guessing and hear it directly. We asked them to walk us through how they actually handled photos on a job — not how they thought they should, but what they really did. Almost everyone described a workflow that routed around JobNimbus entirely.
            </p>
            <p className="text-[#a8d8a8] leading-relaxed text-sm max-w-2xl">
              What came back was consistent and specific. The quotes below shaped the direction of everything that followed.
            </p>
            <div className="space-y-4 max-w-2xl">
              {[
                { quote: "That's, to me, the biggest thing that JobNimbus isn't doing. Now, I know they do annotations and all that stuff, but it's the link.", name: "Dan", role: "Sales Lead, Axis Roofing" },
                { quote: "My theory is I get off the roof, I send them a link, they sell themselves on the roof.", name: "Dan", role: "Axis Roofing" },
                { quote: "Pick two per page and it covers the whole page… The picture becomes the message.", name: "Ovid Figueroa", role: "Salesperson, Muto Inc" },
                { quote: "If there's 20 pictures, then when I create the PDF from that form, they're like super tiny pictures because they're fitting all 20 pictures on one page.", name: "Kelsey Smith", role: "Office Manager, Thorntons Roofing" },
                { quote: "Honestly, the JobNimbus one looks like absolute trash. I'd rather write it with pen and paper and mail it with a pigeon.", name: "Doron Waldman", role: "Owner, 911 Restoration" },
                { quote: "JobNimbus holds our photos. That's all it does. We do nothing inside JobNimbus photos.", name: "Carmen", role: "Field Lead, Results Roofing" },
                { quote: "There's not just before and after, there's progress, there's in between… all the contractors hate it.", name: "Doron Waldman", role: "911 Restoration" },
                { quote: "We've been called out saying that wasn't the right house, but [with] Company Cam it has a location on there… so we're able to use that as a rebuttal, saying we're actually here and here's the time we actually did it.", name: "Carmen", role: "Results Roofing" },
                { quote: "You can't email anything bigger than 25 megabytes. How do you do that for a photo report that has over 100 photos in it?… The idea is that they never, ever, ever click on links.", name: "Internal", role: "Mobile Story Workshop" },
                { quote: "Every project we've ever done since 2018 is in there. If I cancel my subscription, I lose it.", name: "Bill", role: "Ops, Axis Roofing" },
              ].map(({ quote, name, role }) => (
                <div key={name + quote.slice(0, 20)} className="border-l-2 border-[#2a4a2a] pl-4 py-1">
                  <p className="text-[#c8e6c8] text-sm leading-relaxed italic mb-2">&ldquo;{quote}&rdquo;</p>
                  <p className="text-xs text-[#6b8f6b]">— {name}, {role}</p>
                </div>
              ))}
            </div>
            <div className="w-full rounded-xl overflow-hidden border border-[#1e2e1e]" style={{aspectRatio: '16/7'}}>
              <iframe
                src="https://www.figma.com/embed?embed_host=share&url=https%3A%2F%2Fwww.figma.com%2Fboard%2Fl6UwNcPXswo0LZvTAFliqa%2FInspections-and-Photo-Reports%3Fnode-id%3D0-1%26t%3Do3T1yT1b5J4MBK6r-1"
                allowFullScreen
        ...
        {activeSub?.title === "Photo Reports" && (
          <div className="mt-12 space-y-6">
            ...
            <video
              ...
              controls
              className="w-full rounded-xl aspect-video bg-[#161c16]"
            />
          </div>
        )}
      ...
      {/* Outcomes */}
      {(() => {
        const outcomes = activeSub ? activeSub.outcomes : project.outcomes;
        if (activeSub && !outcomes) return null;
        return (
          <section className="mb-20">
            ...
            {(outcomes?.stats?.length ?? 0) > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                {outcomes!.stats.map(({ stat, label }) => (
            ...
            {outcomes?.reflectionHeading || outcomes?.reflectionItems ? (
              <div className="max-w-2xl space-y-4">
                {outcomes.reflectionHeading && (
                  <h3 className="text-base font-semibold text-[#c8e6c8]">{outcomes.reflectionHeading}</h3>
                )}
                {outcomes.reflectionItems && (
                  <ul className="space-y-3">
                    {outcomes.reflectionItems.map((item, i) => (
                ...
                {outcomes?.reflection && (
                  <p className="text-[#a8d8a8] leading-relaxed text-sm">{outcomes.reflection}</p>
                )}
              </div>
            ) : (
              <p className="text-[#a8d8a8] leading-relaxed text-sm max-w-2xl">
                {outcomes?.reflection ?? "Summarize the impact: what changed after this shipped? What did you learn? What would you do differently?"}
              </p>
            )}
          </section>
        );
